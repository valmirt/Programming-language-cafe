...
function analisador_semantico (file, regra, terminal)
	...
	elseif regra == 11 then
		...
		if terminal.tipo == 'int' then
			file = file..'scanf("%s", '..termial.lexema..');\n'
		elseif terminal.tipo == 'real' then
			file = file..'scanf("%d", &'..termial.lexema..');\n'
		elseif terminal.tipo == 'lit' then
			file = file..'scanf("%1f", &'..termial.lexema..');\n'
		else
			print('Erro!! Variável '..termial.lexema..' não declarada...')
			erro = true
			erro_semantico = true
		end
	elseif regra == 12 then
		print('Gerar código para o comando escreva')
		file = file..'printf("'..nao_terminais.ARG.lexema..'");\n'
